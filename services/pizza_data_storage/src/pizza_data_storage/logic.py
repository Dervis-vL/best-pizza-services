...
from pizza_data_storage import repositories, settings, utils

logger = logging.getLogger(__name__)


def seed_categories_and_editions(config: shared_schemas.RankedCategoriesSchema) -> None:
    """Seed the database with the ranking categories and editions from a schema."""

    ranking_repo = repositories.RankingsRepository.from_settings(
        db_settings=settings.pizza_db
    )
    ranking_repo.upsert_categories_and_editions(config=config)


def scrape_editions() -> Generator[shared_schemas.PizzeriaEndpointsSchema, None, None]:
...
def parse_pizzerias_and_webpages(config: shared_schemas.PizzeriaEndpointsSchema) -> None:
...
def scrape_pizzeria_pages() -> Generator[shared_schemas.LocationSchema, None, None]:
...
def parse_pizzeria_page(location_schema: shared_schemas.LocationSchema) -> None:
...
def scrape_data_from_url(url: str) -> bs.BeautifulSoup:
    ...
    try:
        ...
        logging.warning("HTTP %s fetching %s: %s — skipping", e.code, url, e.reason)
    except URLError as e:
        logging.warning("Failed to reach %s: %s — skipping", url, e.reason)
    except TimeoutError:
        logging.warning("Timeout fetching %s — skipping", url)
