...
def get_category_payload_repo(engine: EngineDep) -> storage_repos.CategoryPayloadRepository:
    """Return a CategoryPayloadRepository from the request-scoped engine."""
    return storage_repos.CategoryPayloadRepository.from_engine(engine=engine)
...
CategoryPayloadRepoDep = Annotated[
    storage_repos.CategoryPayloadRepository,
    Depends(get_category_payload_repo),
]
